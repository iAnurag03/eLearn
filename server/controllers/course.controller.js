...
export const createCourse = async (req, res) => {
  try {
    ...
    console.log("Creating course for user ID:", req.id);
    ...
    console.log("Course created with ID:", cid);
    console.log("User ID:", uid);

    // Add instructor to enrolled students and enrolled courses
    const courseUpdateResult = await Course.updateOne(
      { _id: cid },
      { $push: { enrolledStudents: uid} }
    );
    console.log("Course update result:", courseUpdateResult);

    const userUpdateResult = await User.updateOne(
      { _id: uid },
      { $push: { enrolledCourses: cid} }
    );
    ...
    // Create a CoursePurchase record for the instructor (free access)
    const purchaseRecord = await CoursePurchase.create({
      courseId: cid,
      userId: uid,
      amount: 0, // Free for instructor
      status: "completed", // Automatically completed
      paymentId: `instructor_access_${Date.now()}`, // Unique identifier for instructor access
    });
